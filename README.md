...
- Live interruption/recovery latency is **measured** — fence ~2.5s median / 4.6s p95, recovery ~4s median / 7.3s p95 over a 20-interruption real call. Recovery latency (LLM + TTS round-trip) is slower than we'd like and is a live optimization target, not something tuned away before reporting. Full table and disclosed caveats in `apps/agent/src/bench/live-latency.md`.
