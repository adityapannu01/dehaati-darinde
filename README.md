...
- **Multilingual (`RIME_MULTILINGUAL=true`) is degraded for non-English and only `eng`+`hin` are tested.** The §0 spike (recorded in `RIME_EVIDENCE.md §4a`) found Coda emits word-level timestamps only for English — synthesised Hindi and Japanese returned audio but zero timed words. A second ROUND3 spike checked whether the Rime plugin's per-segment boundary frame could stand in for non-English and found it can't: `ws3` sends one stream-level `done`, not one boundary per sentence. So a non-English turn loses the per-sentence commit gate and falls back to `onTurnComplete` granularity (an interruption drops everything pending). The HUD shows `degraded timing`. English keeps the full gate. The other 7 Coda languages are mapped in `voices.ts` but their voices and segmentation are untested. It is not a translation feature — the agent replies in the room's language, it never translates.
